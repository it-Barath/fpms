...
if ($_SESSION['user_type'] === 'gn') {
    ...
    if ($verify_result->num_rows === 0) {
        ...
        echo json_encode(['success' => false, 'message' => 'Access denied to this family']);
        exit();
    }
}
...
try {
    ...
    if ($is_edit) {
        ...
        $update_query = "UPDATE form_submissions_family SET is_latest = 0 WHERE submission_id = ?";
        $update_stmt = $db->prepare($update_query);
        $update_stmt->bind_param("i", $submission_id);
        $update_stmt->execute();
        
        // Get version from old submission
        $version_query = "SELECT version FROM form_submissions_family WHERE submission_id = ?";
    ...
    $insert_submission_query = "INSERT INTO form_submissions_family ($submission_columns) VALUES ($submission_placeholders)";
    $insert_submission_stmt = $db->prepare($insert_submission_query);
    
    $param_types = str_repeat('s', count($submission_data));
    ...
    foreach ($fields as $field) {
        ...
        if ($field_type === 'file') {
            ...
            if ($remove_file) {
                ...
                $file_name = 'form_' . $form_id . '_' . $family_id . '_' . time() . '_' . $field_id . '.' . $file_ext;
                $file_path = $upload_dir . $file_name;
                
                // Move uploaded file
                if (move_uploaded_file($file['tmp_name'], $file_path)) {
                    $relative_path = 'assets/uploads/forms/' . $file_name;
                ...
                $existing_query = "SELECT * FROM form_responses_family WHERE submission_id = ? AND field_id = ?";
        ...
        if (isset($response_data)) {
            ...
            $insert_response_query = "INSERT INTO form_responses_family ($response_columns) VALUES ($response_placeholders)";
            $insert_response_stmt = $db->prepare($insert_response_query);
            
            $response_param_types = str_repeat('s', count($response_data));
    ...
    $update_completed_query = "UPDATE form_submissions_family SET completed_fields = ? WHERE submission_id = ?";
    ...
    $table_name = 'form_submissions_family';
