...
switch ($_SESSION['user_type']) {   
    ...
    case 'district':
        $stats = $userManager->getDistrictStats($_SESSION['office_code']);
        $recentActivities = $userManager->getRecentActivities(10, $_SESSION['office_code']);
        break;
        
    case 'division':
        $stats = $userManager->getDivisionStats($_SESSION['office_code']);
        $recentActivities = $userManager->getRecentActivities(10, null, $_SESSION['office_code']);
        break;
        
    case 'gn':
        $stats = $userManager->getGnStats($_SESSION['office_code']);
        $recentActivities = $userManager->getRecentActivities(10, null, null, $_SESSION['office_code']);
        break;
}
...
<div class="col-xl-3 col-md-6 mb-4">
    <div class="card text-white bg-purple h-100">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h6 class="card-title mb-1">Form Management</h6>
                    <?php 
                    $userManager = new UserManager();
                    $formStats = $userManager->getFormSubmissionStats();
                    ?>
                    <h2 class="card-text mb-0"><?php echo $formStats['total_submissions']; ?></h2>
                </div>
                <i class="fas fa-file-alt fs-1 opacity-75"></i>
            </div>
        </div>
        <div class="card-footer d-flex align-items-center justify-content-between">
            <a href="forms/available.php" class="text-white small">View Forms</a>
            <span class="small">
                <?php echo $formStats['pending_reviews']; ?> Pending
            </span>
        </div>
    </div>
</div>
